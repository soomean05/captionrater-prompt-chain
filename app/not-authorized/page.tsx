...
export default async function NotAuthorizedPage({
  ...
  const profile = gate?.profile ?? null;
  return (
      ...
      <div className="mx-auto flex min-h-screen max-w-lg flex-col justify-center px-6 py-12">
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
          ...
          <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">
            ...
            <span className="font-medium">{gate.user?.email ?? "unknown"}</span>
          </p>
          {isDev ? (
            <div className="mt-4 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-300">
              <p className="font-semibold">Temporary auth debug (dev only)</p>
              <p className="break-all">auth user id: {gate?.user?.id ?? "none"}</p>
              <p className="break-all">auth email: {gate?.user?.email ?? "none"}</p>
              <p className="break-all">profile id: {profile?.id ?? "none"}</p>
              <p className="break-all">profile email: {profile?.email ?? "none"}</p>
              <p>
                is_superadmin raw:{" "}
              ...
              <p>
                ...
                {gate?.normalized
                  ? String(gate.normalized.is_superadmin)
                  : "none"}
              </p>
              <p>
                normalized is_matrix_admin:{" "}
                {gate?.normalized
                  ? String(gate.normalized.is_matrix_admin)
                  : "none"}
              </p>
              <p>final allowed: {String(gate?.allowed ?? false)}</p>
            </div>
          ) : null}
